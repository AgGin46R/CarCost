            ...
            when {
                ...
                remoteCar == null -> {
                    if (localCar.id !in ownedCarIds) {
                        // Машины нет на сервере, а мы не владелец — значит её
                        // удалил владелец. Отправить её заново означало бы
                        // воскресить удалённое, причём и у него тоже.
                        // Проверка роли нужна именно здесь, а не на правке
                ...
                // Автомобиль изменён локально позже — отправляем.
                //
                // Проверки «я владелец» здесь больше нет, и это не послабление.
                // Совладелец правит машину и так: форма редактирования и
                // пересчёт пробега по расходам отправляют изменения напрямую,
                // никого не спрашивая, и сервер их принимает — политика
                // cars_update_members разрешает правку любому участнику.
                // Проверка стояла только на этом пути — на повторной отправке
                // после сбоя. Получалось, что правка совладельца, не уехавшая
                // с первого раза (не было сети), не уезжала уже никогда:
                // здесь её пропускали, а других попыток не было. Изменение
                // оставалось на телефоне навсегда, и никто об этом не узнавал.
                //
                // От затирания чужого защищает не роль, а условие выше:
                // отправляем, только если локальная версия действительно
                // новее серверной.
