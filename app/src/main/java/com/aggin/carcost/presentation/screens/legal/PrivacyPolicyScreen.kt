...
private val PRIVACY_SECTIONS: List<Pair<String, String>> = listOf(
    ...
    "Кому передаются данные" to
        ...
        "RuStore Push — доставка push-уведомлений на устройствах без сервисов " +
        "Google. Передаётся только идентификатор устройства для доставки.\n\n" +
        "MyTracker — статистика использования: запуски приложения, версия, " +
        "модель устройства. Нужна, чтобы понимать, какими разделами пользуются, " +
        "а какие лишние. Ваши записи, чаты и файлы в статистику не входят.\n\n" +
        "Tracer — сведения о сбоях и зависаниях приложения: модель устройства, " +
        "версия Android, версия приложения и место в коде, где произошёл сбой. " +
        "Отправляется только при неполадке и нужно, чтобы её починить.\n\n" +
