...
class FuelConsumptionCalculatorTest {
    ...
    fun `длинный отрезок весит больше короткого`() {
        // Отсчёт начинается не с нуля: запись с нулевым одометром расчёт
        // отбрасывает намеренно — ноль означает «пробег не заполнен», а не
        // «новая машина». Этот тест писался с нулём и падал с первого дня,
        // но запустить его в этом окружении было нельзя.
        val expenses = listOf(
            fill(odometer = 10_000, liters = 10.0),
            fill(odometer = 10_100, liters = 10.0),  // 10 л/100км на 100 км
            fill(odometer = 11_100, liters = 60.0)   // 6 л/100км на 1000 км
        )
