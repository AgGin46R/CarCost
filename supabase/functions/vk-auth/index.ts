...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      //
      // vk_id здесь НЕ пишется намеренно. Это признак «аккаунт ЗАВЕДЁН через
      // ВКонтакте», по нему приложение решает, можно ли отвязывать VK: у
      // заведённого через VK другого способа войти нет, и отвязка отняла бы
      // доступ навсегда — см. isVkAccount() в SupabaseAuthRepository.
      //
      // Раньше vk_id проставлялся здесь, то есть при КАЖДОМ входе. В результате
      // человек, привязавший ВК к своему обычному аккаунту с почтой и паролем,
      // после первого же входа через ВК помечался как «заведён через ВК», и
      // кнопка «Отвязать» у него пропадала навсегда. Поймано на живом аккаунте.
      //
      // Ставится vk_id только в createAuthUser, при создании. Метаданные в
      // GoTrue дописываются, а не заменяются (проверено), поэтому обновление
      // без vk_id у заведённых через VK его не сотрёт.
